...
void June_2018_TB()
{
  ...
  const int numOfBoards = 28;
  int Energy[numOfEnergy] = {150,100,80,50,30,10};

  // for (int i=0;i<numOfEnergy;i++)
  // {
  //   a = new SaturationFinder(numOfBoards,Energy[i],"Electrons","June_2018_TB","June_2018","/eos/cms/store/group/dpg_hgcal/tb_hgcal/2018/cern_h2_june/offline_analysis/ntuples/v6");
  ...
  // For all electrons
  a = new SaturationFinder(numOfBoards,0,"Electrons","June_2018_TB","June_2018","/eos/cms/store/group/dpg_hgcal/tb_hgcal/2018/cern_h2_june/offline_analysis/ntuples/v6");
  a->CreateStructure("/eos/user/r/rsaradhy/www/June2018/");
  a->SetModuleMaps("forTBFramework/2018/layerGeom_june2018_h2_28layers.txt");
  // a->CollectFromNTuples(runFileFolder); //Not a good idea for Electrons all as it will eat up ram!!! use:hadd ./All_Electrons/June_2018_Energy_0_Electrons.root */*.root instead...
  // a->SaveHistogram(); //Save histograms for faster processing later on
  a->LoadHistogram(); // Load the saved histograms...
  a->FitHistogram(); // Fit those histograms Make sure that "gROOT->SetBatch(kTRUE); //Not displaying anything" is not commented
  a->FindValues(); //Find the saturation values that we are interested in ...
