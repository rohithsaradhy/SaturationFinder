...
void plot_ModuleStability_TP()
{
  ...
  graphHeader.str("");
  graphHeader<<"Comparisons of Transition Point";
  ...
  for(int i=0; i<6;i++)
  {
    ...
    for(int board=0;board<a[i]->BOARD;board++)
    {
      for(int skiroc=0;skiroc<4;skiroc++)
      {
        ...
        X_Err[count] = 0;
        Y[count] = a[i]->TP[board][skiroc][0];
        Y_Err[count] = a[i]->TP_Err[board][skiroc][0];
        count++;
      }
    }
  ...
  mg1->GetYaxis()->SetRangeUser(500,3500);
  ...
  mg1->GetYaxis()->SetTitle("Transition Point (HG #rightarrow LG)");
  gPad->Update();
  gPad->SetGridx();
  gPad->SetGridy();
  gPad->SaveAs("Canvas1_TP.png");
  ...
  mg2->GetYaxis()->SetRangeUser(500,3500);
  ...
  mg2->GetYaxis()->SetTitle("Transition Point (HG #rightarrow LG)");
  gPad->Update();
  gPad->SetGridx();
  gPad->SetGridy();

  gPad->SaveAs("Canvas2_TP.png");
