...
void plot_Stability_TP()
{
  ...
  TCanvas* TP_Canvas = new TCanvas("TP","TP",1366,768);
  ...
  outputFile.str();
  outputFile<<"./Analysed_data/April/H2_TP_Stability.png";
  graphHeader.str("");
  ...
  for(int board=0;board<a->BOARD;board++)
  {
    for(int skiroc=0;skiroc<4;skiroc++)
    {
      X[count] = count;
      X_Err[count] = 0;
      Y[count] = a->TP[board][skiroc][0];
      Y_Err[count] = a->TP_Err[board][skiroc][0];
      count++;
    }
  }
